...
class BCDUNet(nn.Module):
    def __init__(
        ...
        self.upconv3 = nn.ConvTranspose2d(
            num_filter * 8, num_filter * 4, kernel_size=2, stride=2
        )
        self.upconv2 = nn.ConvTranspose2d(
            num_filter * 4, num_filter * 2, kernel_size=2, stride=2
        )
        self.upconv1 = nn.ConvTranspose2d(
            num_filter * 2, num_filter, kernel_size=2, stride=2
        )
        ...
        if bidirectional:
            self.clstm1 = ConvBLSTM(
                num_filter * 4,
                num_filter * 2,
                (3, 3),
                (1, 1),
                "tanh",
                list(self.frame_size // 4),
            )
            self.clstm2 = ConvBLSTM(
                num_filter * 2,
                num_filter,
                (3, 3),
                (1, 1),
                "tanh",
                list(self.frame_size // 2),
            )
            self.clstm3 = ConvBLSTM(
                num_filter,
                num_filter // 2,
                (3, 3),
                (1, 1),
