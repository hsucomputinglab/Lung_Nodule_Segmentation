...
class ConvLSTM(nn.Module):
    ...
    def forward(self, X):
        ...
        # Initialize output
        output = torch.zeros(
            batch_size, seq_len, self.out_channels, height, width, device=self.device
        )
...
class ConvBLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        y_out_reverse = self.backward_cell(x[:, reversed_idx, ...])[
            :, reversed_idx, ...
        ]
